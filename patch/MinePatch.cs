...
namespace sogs_standing_on_giants_shoulders_a_collection_of_physics_improv.patch
{
    ...
    class MinePatch
    {
        ...
        static void DPSStartPatch(DeepMiner __instance)
        {
            ...
            if ((bool)StaticAttributes.configs["DeepMinerRegions"]) {
                ...
                new Thread(() =>
                {
                    ...
                    try
                    {
                        ...
                        int n = 0;
                        for (int i = (int)(StaticAttributes.mineConfigsFloat["rangeDP"] * -1f); i < StaticAttributes.mineConfigsFloat["rangeDP"] - 1; i++)
                        {
                            SOGS.log("MinePatch :: DPSStartPatch --> " + __instance.ReferenceId + " i " + i, SOGS.Logs.DEBUG);

                            for (int k = (int)(StaticAttributes.mineConfigsFloat["rangeDP"] * -1f); k < StaticAttributes.mineConfigsFloat["rangeDP"] - 1; k++)
                            {
                                SOGS.log("MinePatch :: DPSStartPatch --> " + __instance.ReferenceId + " k " + k, SOGS.Logs.DEBUG);

                                for (int j = (int)vec.y; j < Math.Abs(vec.y - WorldManager.BedrockLevel - 1); j++)
                                {
